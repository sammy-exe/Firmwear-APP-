...
export default function MaxDesk() {
  const navigate = useNavigate();
  const [pressed, setPressed] = useState(null);

  function handlePress(mode) {
    setPressed(mode);
    setTimeout(() => { setPressed(null); navigate(`/maxdesk/${mode}`); }, 150);
  }
    ...
    <div className="screen-wrap">
      ...
      <div className="top-nav">
        <div className="nav-left">
          ...
          <div className="nav-title">MaxDesk</div>
        </div>
        <ThemeToggle />
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, padding: '0 13px', position: 'relative', zIndex: 2, flex: 1, alignItems: 'center' }}>
        <ModeCard
          emoji="🖼️" label="ShowCase" desc="Loop images on bot screen"
          color="#00bfa8"
          pressed={pressed === 'showcase'}
          onPress={() => handlePress('showcase')}
        />
        <ModeCard
          emoji="💬" label="AI Assistant" desc="Bot talks to visitors automatically"
          color="#f05c2a"
          pressed={pressed === 'ai'}
      ...
      <div style={{ textAlign: 'center', padding: '8px 0 10px', position: 'relative', zIndex: 2, flexShrink: 0 }}>
        <button onClick={() => navigate('/datastore')} style={{ fontSize: 9, color: 'var(--teal-d)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700 }}>⬡ Data Store</button>
      </div>
    </div>
  );
}
